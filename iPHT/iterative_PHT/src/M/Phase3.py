...
class Phase3:
    ...
    def distance_3d(point1, point2):
        point1_real = np.real(point1)
        point2_real = np.real(point2)
        return np.sqrt(
            (point2_real[0] - point1_real[0]) ** 2 +
            (point2_real[1] - point1_real[1]) ** 2 +
            (point2_real[2] - point1_real[2]) ** 2
        )
    #####################################################################
    ...
    def get_t(w, xq, yq, zq):
        ...
        if three_real:
            ...
            for k in range(0, len(root)):
                t_dists.append(Phase3.distance_3d(point, closest_points[k]))
        ...
        return best_root_real #best_root.astype(float)
    ...
    #####################################################################
    def Curve_Loss_Function2(w, a1, b1, c1, a2, b2, c2, a3, b3, c3, xq, yq, zq, ang, vectors, Hough_Accumulate, xxx, yyy, zzz, w_1, w_2, Use_l, k):
        ...
        for i in range(len(xq)):
            ...
            if three_real:
                ...
                t_dists = []
                for k in range(0, len(root)):
                    t_dists.append(Phase3.distance_3d(point, closest_point[k]))
            ...
            # If there is only one, use that to calculate the distance
                ...
                closest_point = Phase3.func(w, np.array([[0], [1], [2]]), root)
                distance = Phase3.distance_3d(point, closest_point)
                best_root = roots[real_index]
            ...
            best_root_real = np.real(best_root)
            v1_t = Phase3.derivative_curve_equations(b1, c1, b2, c2, b3, c3, best_root_real)
        ...
        loss_a = sum_probabilities * w_2
        loss_d = sum_distances * w_1
        loss_function = loss_d - loss_a
        return sum_probabilities, sum_distances, loss_function, loss_d, loss_a
    ...
    #####################################################################
    def polynomial_regression3d(x, y, z, degree, test=True):
    ...
    def curve_limit_distance(x_best, start, stop, num_points = 100):
        ...
        for i in range(len(par)):
            if (i+1 < len(par)):
                ...
                #y[num_points]=y2
                #z[num_points]=z2
                distance = distance + Phase3.distance_3d([x[i],y[i],z[i]],[x2,y2,z2])
    ...
    def curve_limit_distance_new(x_best, start, stop, num_points = 100):
        ...
        for i in np.linspace(start, stop, num_points):
            if (i+1 < num_points):
                ...
                distance = distance + Phase3.distance_3d([x,y,z],[x2,y2,z2])
