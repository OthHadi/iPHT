...
from SP.Polyhedra import Polyhedra
...
class Phase2:
    ...
    def polyhedra_octahedral_old(self, radius, num=2, plot_me=0):
        ...
        if (plot_me == 1):
            ...
            # #ax.plot3D(x[np.where(x <= 0)], y[np.where(x <= 0)], z[np.where(x <= 0)], 'r.', markersize=markersize_value)
            #print("angles number=", len(x_new))
            #ax.plot3D(x_new, y_new, z_new, 'r.', markersize=markersize_value)
            #plt.show()
    ...
    def polyhedra_octahedral_old(self, radius):
        ...
        # #ax.plot3D(x, y, z, 'r.', markersize=markersize_value)
    ...
    def Convex_Hull_JM(self, data, radius):
        ...
        view_lim2 = -0.5
        markersize_value = 10
        #fig = #plt.figure(figsize=(figsize_value, figsize_value2))
        #plt.plot(data[:, 0], data[:, 1], 'or', markersize=markersize_value)
        ##plt.annotate(str(i), (data[i, 0], data[i, 1]), fontsize=12, color='k')
        #plt.xlim(view_lim2, view_lim)
        #plt.ylim(view_lim2, 0.5)
        #plt.grid()
        #plt.xlabel(r'$\omega_1$')
        #plt.ylabel(r'$\omega_2$')
        #plt.gca().set_aspect('equal')

        # Find the lowest value in the y-coordinate and add it as the first point in the convex hull
        ...
        while (True):
            ...
            #plt.plot([a_x, b_x], [a_y, b_y], 'k', linestyle='-')
            # #plt.pause(0.2)
            ...
            for c in range(len(data[:, 0])):
                ...
                if ((dis <= radius) and (dis != 0) and (self.check_number_of_exist(2, convex_hull, c) <= 2)) == True:
                    ...
                    #print('c=', c, ', ', c_i1, '!=', c_i2, ', ', c_i3, '!=', c_i4)
                    # if ((c_i1 != c_i2 and c_i3 != c_i4) == False) or (test_break == 0):
                    if (c_i1 + c_i2) != 0 or (c_i3 + c_i4) != 0 or test_break <= 1:
                        ...
                        if (sub >= 0):
                            ...
                            test_angle = 360 + sub
                        #print('c=', c, ', test_angle = ', line_B, '-', line_A1, '=', sub)
                        #print(self.check_number_of_exist(1, convex_hull, c))
                        ...
                        #print('smallest_angle = ', smallest_angle)
            ...
            # save the first and the second point to use them for stopping the method
            if (test_break == 0):
    ...
    def Convex_Hull_P3(self, C_X2, C_Y2, radius):
        ...
        while (True):
            ...
            #print('flip=', flip)
            #print('c =', convex_hull_last_input)
    ...
    def Convex_Hull2(self, C_X, C_Y, graph, next_convex_hull):
        ...
        coordinate = 'y'
        while(True):
        # Find the lowest value in the y-coordinate
            convex_hull = []
            if (coordinate == 'y'):
                min_C_Y = max(C_Y)
            ...
            main_break = 0
            #while (main_break < 9):
            #    if(convex_hull_first_input != convex_hull_last_input):
            #        main_break += 1
    ...
    def get_angle2(self, x1, y1, x2, y2):
        if ((x1<=x2) and (y1<=y2)) == True : # 1st quadrant
            ...
            #print("(abs(",y2," - ",y1,"), abs(",x2," - ",x1,")) = ",angle)
            #print("1st quadrant")
        elif ((x1>=x2) and (y1<=y2)) == True :# 2nd quadrant
            angle = (math.pi/2) + math.atan2(abs(x2 - x1), abs(y2 - y1))
            #print("90 + (abs(",x2," - ",x1,"), abs(",y2," - ",y1,")) = ",angle)
            #print("2 quadrant")
        elif ((x1>=x2) and (y1>=y2)) == True :# 3rd quadrant
            angle = (math.pi) + math.atan2(abs(y2 - y1), abs(x2 - x1))
            #print("180 + (abs(",y2," - ",y1,"), abs(",x2," -(abs(",y2," - ",y1,"), abs(",x2," - ",x1,")) ",x1,")) = ",angle)
            #print("3 quadrant")
        elif ((x1<=x2) and (y1>=y2)) == True :# 4th quadrant
            angle = (3*math.pi/2) + math.atan2(abs(x2 - x1), abs(y2 - y1))
            #print("270 + (abs(",x2," - ",x1,"), abs(",y2," - ",y1,")) = ",angle)
            #print("4 quadrant")
        else:
            print("what")
    ...
    def Convex_Hull(self, C_X, C_Y, graph):
        ...
        # Add it to the convex hull's vector
        convex_hull.append(index_C_Y)
        ...
        while (convex_hull_first_input != convex_hull_last_input):
            ...
            convex_hull.append(index_c)
        '''
        for c in range(3,3-len(C_X),1):
            angle2 = get_angle(C_X[c-2], C_Y[c-2], C_X[c-1], C_Y[c-1])
            angle1 = get_angle(C_X[c-1], C_Y[c-1], C_X[c], C_Y[c])
