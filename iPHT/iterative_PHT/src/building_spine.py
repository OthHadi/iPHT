...
###############################################################################################################################################
                ...
                for MOO in range(multi_objective_optimization_range.shape[0]):
                    ...
                    start_in = Phase3.get_t(x_best, xq[first_end], yq[first_end], zq[first_end])
                    stop_in = Phase3.get_t(x_best, xq[second_end], yq[second_end], zq[second_end])
                    sum_edge_distance, x_plot,y_plot,z_plot = Phase3.curve_limit_distance(x_best, start_in, stop_in)
                    #sum_edge_distance = 0
                    ...
                    while k < k_max:
                        ...
                        if ((prob > rand) and (e_new < e_best)) == True:
                            ...
                            if (sum_edge_distance*1.1 >= distance):
                                count_itr += 1
                                x = x_new
                                loss_function = e_new
                        ...
                        if e_new < e_best:
                            ...
                            stop_in = Phase3.get_t(x_new, xq[second_end], yq[second_end], zq[second_end])
                            distance, x_plot, y_plot, z_plot = Phase3.curve_limit_distance(x_new, start_in, stop_in)
                            if (sum_edge_distance * 1.1 >= distance) and (loss_d < best_fetted_curve):   # add this
